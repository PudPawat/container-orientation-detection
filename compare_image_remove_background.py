...
from lib.contour_after_process import contour_area, contour_center_dis,contour_center_X_or_Y
...
class OrientationDetection():
    def __init__(self, path = "", json_path = ""):
        ...
        self.crop_circle_fix_r = self.opt.crop_circle_fix_r
        self.crop_circle_platform = self.opt.crop_circle_platform

        self.threshold_score = self.opt.threshold_score

        self.object_contour_area_min = self.opt.object.object_contour_area_min
        self.object_contour_area_max = self.opt.object.object_contour_area_max
        self.object_contour_distance_criterion = self.opt.object.object_contour_distance_criterion
        self.circle_radius_minus_safety = self.opt.object.circle_radius_minus_safety  ### to removing noise
        self.text_color = self.opt.text_color

        self.notch_dis_criterion = self.opt.notch.notch_dis_criterion
        self.notch_dis_from_right_edge = self.opt.notch.notch_dis_from_right_edge
        self.notch_area_min = self.opt.notch.notch_area_min
...
if __name__ == '__main__':
    ...
    for j in range(len(names)):
        ...
        # result = cv2.putText(img2,)
        if angle is not None:

            result = cv2.putText(img2, "ANGLE: {}".format(str("%.2f" % round(angle, 2))),
                                  (0, img2.shape[0] - 10), cv2.FONT_HERSHEY_COMPLEX, 8, (0,50,255), 8)
