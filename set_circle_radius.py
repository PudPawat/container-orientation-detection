...
class set_radius_circle():
    def __init__(self):
        pass


def update_circle_parameters(x):
    global circle_x, circle_y, radius

    # Get the current trackbar values
    circle_x = cv2.getTrackbarPos("Center X", "Image")
    circle_y = cv2.getTrackbarPos("Center Y", "Image")
...
if __name__ == '__main__':
    ...
    for name in names:
        ...
        try:
            ...
            print(config)
            circle_x, circle_y, radius = config["crop_circle_platform"]
        ...
        if circle_x is None:
            circle_x, circle_y, radius = 0, 0, 0

        circle_normal = (circle_x, circle_y, radius)

        while (1):

            cv2.imshow('Image', img)

            cv2.createTrackbar("Center X", "Image", int(circle_x), img.shape[1], update_circle_parameters)
            cv2.createTrackbar("Center Y", "Image", int(circle_y), img.shape[0], update_circle_parameters)
            cv2.createTrackbar("Radius", "Image", int(radius), int(min(img.shape[0], img.shape[1]) / 2), update_circle_parameters)

            circle_new = (circle_x, circle_y, radius)

            if radius > 0:
                temp_image = img.copy()
                cv2.circle(temp_image, (circle_x, circle_y), radius, (0, 255, 0), 2)
                ...
                circle_resize = [(int(circle_normal[0][0]/0.3),int(circle_normal[0][1]/0.3)), int(circle_normal[1]/0.3)]
